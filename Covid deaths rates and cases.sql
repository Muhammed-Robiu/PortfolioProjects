...
select continent, max(total_cases) as [cases per continent]
from portfolioproject..CovidDeaths
where continent is not null
group by continent
order by 2;
...
create view [death rate per continent] as
select continent, sum(cast(total_deaths as int)) as [death rate per contintent]
from portfolioproject..CovidDeaths
where continent is not null
group by continent;

create view [cases per continent] as
select continent, max(total_cases) as [cases per contintent]
from portfolioproject..CovidDeaths
where continent is not null
group by continent;

create view [cases per populatiion] as
select location,  date, total_cases, population, (total_cases/population)*100 as [cases per population]
from portfolioproject..CovidDeaths
where total_cases is not null and population is not null;
